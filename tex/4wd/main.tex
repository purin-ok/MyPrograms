...
\begin{figure}[t]
  ...
  \begin{minipage}[]{0.45\hsize}
    \centering
    \includegraphics[width=5cm]{img/media.png}
    \subcaption{MediaPipe Poseで取得できる関節位置}
    \label{RGB}
  \end{minipage}\\
  \begin{minipage}[]{0.45\hsize}
    \centering
    \includegraphics[width=5cm]{img/nuitrack.png}
...
\begin{figure}[b]
  \centering
  \includegraphics[width=5cm]{img/image_3D.png}
...
MediaPipe Poseはノイズはあるが，体に対し平行な向きはある程度正確に測定できることがわかった．
...
\end{lstlisting}
% \chapter{アクティブステレオ法}\label{active}
% アクティブステレオ法とは，\ref{}入力装置から
