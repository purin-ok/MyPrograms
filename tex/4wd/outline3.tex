...
また，慣性式モーションキャプチャデバイスによる計測結果と比較するためのキャリブレーション方法を検討し，定量的な比較を行った．
慣性式モーションキャプチャデバイスによる計測結果では，z軸方向の変位が0から0.4の範囲に収まっているのに対し，RGB画像では変位が-0.6から0.4の範囲になってしまった．
一台のRGBカメラではデスプに誤差が大きいことがわかった．
}
\begin{document}
\maketitle
...
RGBDによる方法では現在問題の修正中であるがRGBDによる方法を用いることで．デスプ情報を元に正しく測定できるのではないかと考えられる．
