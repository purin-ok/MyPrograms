...
#define N2 10 /*Gauss-Seidel法の繰り返し回数*/
#define BUFSIZE 64
...
/*Jacobi法*/
double* J_method(double **A,double *b,int size, int n){
  int i,j,k;
  double L,R;
  double **x;
...
/*Gauss-Seidel法*/
double* GS_method(double **A,double *b,int size, int n){
  int i,j,k;
  double L,R;
  double *x, *xn;
  x = allocVector(size);	/*最新の値*/
  xn = allocVector(size);	/*一つ前の値*/
  /*要素を0で初期化*/
...
int main(int argc, char ** argv){
  ...
  printf("Jacobi法の反復回数を入力\n");
  fgets(buf, sizeof(buf), stdin);
  n1 = atoi(buf);
  printf("Gauss-Seidel法の反復回数を入力\n");
  fgets(buf, sizeof(buf), stdin);
  n2 = atoi(buf);

  /*Jacobi法で解を求める*/
  x = J_method(A,b,Arow,n1);
